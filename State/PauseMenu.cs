...
namespace ForestQuest.State
{
    public class PauseMenu
    {
        ...
        public void Draw(SpriteBatch spriteBatch, GraphicsDevice graphicsDevice)
        {
            ...
            // Update option positions/bounds each draw (resizable windows)
            for (int i = 0; i < _options.Length; i++)
            {
                Vector2 textSize = _font.MeasureString(_options[i]);
                _optionPositions[i] = new Vector2(
            ...
            UiDraw.Panel(spriteBatch, graphicsDevice, new Rectangle((int)popupX, (int)popupY, popupWidth, popupHeight), Color.Gray);
            ...
            for (int i = 0; i < _options.Length; i++)
            {
                ...
                bool hovered = buttonRect.Contains(mousePos) || _selectedIndex == i;
                bool isDown = hovered && mouse.LeftButton == ButtonState.Pressed;
                bool selected = _selectedIndex == i;

                UiDraw.Button(
                    spriteBatch, graphicsDevice, buttonRect, _options[i], _font,
                    hovered, isDown, selected,
                    normal: new Color(60, 60, 60, 200), hover: new Color(40, 40, 40, 210), down: new Color(30, 30, 30, 220), selectedColor: new Color(70, 70, 110, 210),
